...
@Reducer
public struct BathDetailFeature {
    @ObservableState
    public struct State: Equatable {
        ...
        var count: Int { records.filtered(in: currentRange).count(of: .bath) }
        var previousCount: Int { records.filtered(in: previousRange).count(of: .bath) }
        ...
        var trendEntries: [TrendBarChart.Entry] {
            mode.trailingPeriodStarts(from: selectedDate, count: 7).map { start in
                ...
                let count = records.filtered(in: range).count(of: .bath)
        ...
        var comparisonMessage: String {
            ...
            return "\(mode.previousLabel)보다 목욕 횟수가 \(direction)하였습니다."
        }
    }
