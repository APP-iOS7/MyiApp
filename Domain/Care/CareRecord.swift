...
extension [CareRecord] {
    ...
    public func count(of category: CareEvent.Category) -> Int {
        count(where: { $0.event.category == category })
    }

    public var totalMl: Int {
        reduce(0) { total, record in
            switch record.event {
            case let .babyFood(ml), let .formula(ml), let .pumpedMilk(ml): total + ml
            default: total
            }
        }
    }
    ...
    public func totalSleepMinutes(in range: Range<Date>) -> Int {
        reduce(0) { total, record in
        ...
        public static let mocks: [CareRecord] = {
            ...
            return [
                ...
                CareRecord(
                    ...
                    event: .breastfeeding(leftMinutes: 10, rightMinutes: 8)
                ),
                ...
                CareRecord(
                    ...
                    event: .temperature(celsius: 36.5)
                ),
