...
class CaregiverManager: ObservableObject {
    ...
    func loadCaregiverInfo() async {
        ...
        if let caregiver = await loadCaregiver(uid: uid) {
            ...
            let babies = await loadBabies(ids: caregiver.babyIds)
            ...
            await MainActor.run {
                ...
                if let lastSelectedBabyId = caregiver.lastSelectedBabyId,
                    let lastSelectedBaby = babies.first(where: {
                        $0.id.uuidString == lastSelectedBabyId
                    })
                {
    ...
    func saveCaregiverInfo() async {
        ...
        if let doc = try? await userRef.getDocument(),
            let existingCaregiver = Caregiver(document: doc)
        {
            // 기존 데이터 존재하면 유지
            ...
            let caregiver = Caregiver(
                ...
                babyIds: [],
                lastSelectedBabyId: nil
            )
            try? await userRef.setData(caregiver.toFirestoreData(db: db))
            self.caregiver = caregiver
            self.userName = caregiver.name
            self.email = caregiver.email
            self.provider = caregiver.provider
    ...
    private func loadCaregiver(uid: String) async -> Caregiver? {
        ...
        return Caregiver(document: doc)
    }

    private func loadBabies(ids: [String]) async -> [Baby] {
        var babies: [Baby] = []
        for id in ids {
            guard let doc = try? await db.collection("babies").document(id).getDocument(),
                let baby = Baby(document: doc)
            else { continue }
            babies.append(baby)
        }
        return babies
    ...
    func deleteUserData(uid: String) async throws {
        ...
        guard document.exists, let caregiver = Caregiver(document: document) else {
            throw NSError(
                domain: "", code: -1,
                userInfo: [NSLocalizedDescriptionKey: "사용자 데이터를 찾을 수 없거나 디코딩 실패"])
        }

        try await withThrowingTaskGroup(of: Void.self) { group in
            for babyId in caregiver.babyIds {
                ...
                group.addTask {
                    ...
                    guard let babyData = babyDoc.data(),
                        ...
                        let mainCaregiverId = babyData["mainCaregiver"] as? String
                    else {
                        return
                    }
                    ...
                    if mainCaregiverId == uid {
                        ...
                        for collection in collections {
                            let querySnapshot = try await self.db.collection("babies").document(
                                babyId
    ...
    func deleteBaby(_ baby: Baby) async throws {
        guard Auth.auth().currentUser != nil else {
            throw NSError(
                domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "로그인 상태가 아닙니다."])
        }
        ...
        do {
            ...
            guard let babyData = babyDoc.data(),
                ...
                throw NSError(
                    domain: "", code: -1,
                    userInfo: [NSLocalizedDescriptionKey: "아기 데이터를 찾을 수 없습니다."])
            }
    ...
    func registerBabyUUID(_ babyId: String) async throws {
        ...
        guard let doc = try? await babyRef.getDocument(),
            let baby = Baby(document: doc)
        else {
            throw NSError(
                domain: "", code: -1,
        ...
        if babies.contains(where: { $0.id.uuidString == babyId }) {
            throw NSError(
                domain: "", code: -1, userInfo: [NSLocalizedDescriptionKey: "이미 등록된 아기입니다."])
        }
        ...
        do {
            ...
            await MainActor.run {
                if let index = self.babies.firstIndex(where: { $0.id == baby.id }) {
                    self.babies[index].caregiverIds.removeAll { $0 == caregiverId }
                }
            }
        }
    }
