            ...
            if caregivers.isEmpty {
                ...
                ForEach(caregivers, id: \.id) { caregiver in
                    ...
                    .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                        if caregiver.id != baby.mainCaregiver
                            && CaregiverManager.shared.caregiver?.id == baby.mainCaregiver
                        {
        ...
        .alert("보호자 삭제", isPresented: $showingDeleteAlert) {
            Button("삭제", role: .destructive) {
                if let caregiver = selectedCaregiver {
                    Task {
                        try await CaregiverManager.shared.removeCaregiver(
                            baby: baby, caregiverId: caregiver.id)
        ...
        Task {
            do {
                ...
                guard let userData = userDoc.data(),
                    let babiesRefs = userData["babies"] as? [DocumentReference]
                else {
                ...
                guard let babyData = babyDoc.data(),
                    let caregiverRefs = babyData["caregivers"] as? [DocumentReference]
                else {
                        ...
                        let caregivers = babyData["caregivers"] as? [DocumentReference]
                    {
                        for caregiverRef in caregivers {
                            if caregiverIds.contains(caregiverRef.documentID),
                                !loadedCaregivers.contains(where: {
                                    $0.id == caregiverRef.documentID
                                })
                            {
                                if let doc = try? await caregiverRef.getDocument(),
                                    let caregiver = Caregiver(document: doc)
                                {
